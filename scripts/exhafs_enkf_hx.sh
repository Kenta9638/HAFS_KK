...
if [ ${RUN_GSI_VR_ENS} = YES ]; then
  ...
  if [ ${HX_ENS} != YES ]; then
    ...
    if [ ${MIX_DA_FLAG} = YES ]; then  # KKUROSAWA
   #   export ENSID_ORG=$ENSID
   #   export MEMSTR_ORG=$ENSID
...
if [ ${MIX_DA_FLAG} = YES ]; then           # KKUROSAWA
  if [ $ENSID -le $nens_HAFS ]; then
    ${NCP} ${RESTARTinp}/${PDY}.${cyc}0000.fv_srf_wnd.res.tile1.nc ./fv3_srfwnd # KKUROSAWA
  fi
fi
...
srun  -A aoml-hafsda -p orion --ntasks=40 --ntasks-per-node=10 --cpus-per-task=1 ./hafs_gsi.x 1> stdout 2>&1 # KKUROSAWA
cat stdout
