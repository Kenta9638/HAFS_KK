...
###CONV_VARS[0]="GRLE"
###CONV_VARS[0]="REFD"
NUM_CONV_VARS=${#CONV_VARS[@]}
COUNTER=0
while [ $COUNTER -ne $NUM_CONV_VARS ]; do
  tmp_VAR=${CONV_VARS[$COUNTER]}
  synop_grb2file_binary=${out_prefix}.hafsprs.synoptic.0p03.f${FHR3}.${tmp_VAR}.bin
  ${WGRIB2} ${synop_grb2file} | grep "$tmp_VAR" | ${WGRIB2} -i -no_header ${synop_grb2file} -no_append -ieee $synop_grb2file_binary
...
if [ $SENDCOM = YES ]; then
  ...
  mv *.hafsprs.synoptic.0p03.f*.bin ${COMOUTpost}/ # KKUROSAWA
fi
